import { BOOTSTRAPPING, getAppChanges, LOADING_SOURCE_CODE, MOUNTED, NOT_BOOTSTRAPPED, NOT_LOADED, NOT_MOUNTED, shouldBeActive, UNMOUNTING } from "../applications/app.helpers.js";
import { apps } from "../applications/apps.js";
import { started } from "../start.js";
import './navigation-events.js'
import { capturedEventListeners } from "./navigation-events.js";


function flattenFnArray(fns) { // vue3 路由钩子的组合  koa中的组件 redux中的
...
function toLoadPromise(app) {
    ...
    return Promise.resolve().then(() => {
        // 获取应用的钩子方法 接入协议
        if (app.status !== NOT_LOADED) { // 只有当他是NOT_LOADED 的时候才需要加载
            return app;
        }
...
function toUnmountPromise(app) {
    return Promise.resolve().then(() => { // 如果不是挂载状态 直接跳出
...
export function reroute() {
    ...
    function performAppChanegs() { // 需要调用bootstrap 调用，mount和umnout
        // 应用启动了 需要卸载不需要的应用  卸载
        // ？应用可能没有加载过 (如果没加载，还是需要加载的) => 启动并挂载需要的
        let unmountPromises = Promise.all(appsToUnmount.map(toUnmountPromise)) // 卸载，我就启动
